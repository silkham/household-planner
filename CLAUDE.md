...
**PWA (as-built, Session 4).** `sw.js` is **network-first** (never serves stale code in dev; never caches Supabase/cross-origin), `icon.svg` is the app icon, `manifest.webmanifest` has icons+scope, registered in `app.js`. GitHub Pages deploy still pending (no git remote yet).
...
- **Function hardening still PENDING:** `emma-sheet` is currently anon-callable (accepts the public anon key). Lock it to a signed-in household member before this is production-safe.
