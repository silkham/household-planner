...
**PostgREST 1000-row cap — LANDMINE (Session 11, fixed v1.13.2).** `HP.from(t).select("*")` returns at most **1000 rows** by default. When `category_rules` crossed 1000 (→1067 after the tail import), `loadAll` silently truncated it — saved re-tags landed in the DB but never loaded back into `state.category_rules`, so a merchant stayed "Uncategorised" no matter how many times it was filed (looked like "save does nothing"). Fix: `store.js loadAll` now pages every table via `fetchAllRows` (1000-row `.range` chunks until a short page). **Any table that can exceed 1000 rows MUST be paged** — don't reintroduce a bare `.select("*")` for `category_rules` (or `project_items`, `salary_changes`, etc. as they grow). The symptom is subtle: writes succeed, UI never reflects them.

**Single category_rule write helper (as-built, Session 12; v1.13.1).** `store.saveCategoryRule(match_key, category)` **upserts** on the natural key `(household_id, match_key)`. Use it — NOT `saveRow("category_rules", …)` — for any single-rule write (the Spending categorise sheet + the recurring-flow save both do). The old find-by-match_key-then-insert path crashed with `duplicate key … idx_hp_catrules_key` when a rule for that merchant already existed (stale state, or the existing rule was keyed on a different identity field). `bulkReassignRules` already upserts for the multi-row case.

**Spending month = actuals in the forecast month-row layout (as-built, Session 12; v1.14.0 → v1.16.0).** The Spending tab's selected month is no longer a flat category list — it's a **"position" panel** (`js/spending.js` `buildMonthActuals` + `catGroup`/`merchantRow` + `monthPanelHtml`) that mirrors the forecast table's expanded month row, reusing the `bd-*` / `mr-nums` CSS. Model (LOCKED with the household):
- **Net** header = Income − Spent − Invested (no historical Cash — feed is ~12mo). Summary line shows the three totals.
- **Income** = matched salary streams only (`recurring_flows` income `emma_match_key`, or an `INCOME_RE = /salary|bonus|wages|income/i` category). **Genuine other income** (an inflow whose merchant is NEVER paid anywhere in the feed — family/parent transfers) is a **labelled Income line by source name**. Inflows from a merchant we DO pay = **refunds** → they **net against that merchant's spend line** (a merchant appears in `paidMerchants` = any outflow in the whole feed). So refunds no longer inflate income; e.g. Vionic £364 − £246 − £6 = one £112 line.
- **Out** grouped by category: **General Expenses = every counting outflow EXCEPT recurring categories** (shown vs its `settings.forecast_budgets["General Expenses"]`). **Recurring categories each get their own line** — `recurringCatSet()` = the categories used by expense `recurring_flows` ∪ any managed category named `/recurring|subscription/i` (Vehicle, House - Recurring Payments, Subscriptions - Media, Recurring - Bills/House/Media, etc.). This matches how the forecast engine already groups recurring flows, so it's consistent "across the board" (the engine table's General is a flat budget line — recurring flows are always separate; **no engine change needed**).
- **Drill-down shows per-merchant NET sums, not transactions** (`merchantRow`: name · `N×` · net £, tap to re-file). General drills General → sub-category → merchant sums; recurring groups drill straight to merchant sums. Use the search box for individual txns.
- **Savings & investments** = `investCatSet()` (categories matching `/^transfer\s*-/i` ∪ `accounts.contrib_category`) — a distinct violet section, **counted in Net as "gone"** (their cash but treat it as spent). Plain `Transfers`/`Excluded` stay in **Not counted** (out of Net). CSS: `.sp-month`, `.sp-pos`, `.sp-budget`, `.sp-catoff.invest`, reuses `.bd-cat`/`.bd-cathead`/`.bd-lines`/`.sp-tx`.
- ⚠️ The reconcile "This month" card (`reconcile.js`) still carves recurring only via **linked flows** (not by recurring-category), so a recurring merchant with no flow can still land in its General there. Left as-is (most recurring merchants have flows); align it to `recurringCatSet` if asked.
- **Other income grouping + nested-row indent (Session 13; v1.17.0).** `buildMonthActuals` now splits inflows into matched **salary streams** (each its own Income line, as before) and a single collapsible **"Other income"** group holding every genuine non-salary, non-refund inflow (`{name:"Other income", amount, sources:[{key,name,cat,amount,txns}]}`) — `catGroup` renders it via `incomeSourceList`/`incomeSourceRow` (mint `+£`, tap to re-file). Refund-netting unchanged. CSS: nested `.sp-month .bd-cat .bd-cat` is indented 22px + de-emphasised (weight 400, faint) so General's sub-categories read as children, not siblings of the top-level recurring lines.
- **Reconcile "This month" card gains Other income (Session 13; v1.18.0).** The Forecast card's income was previously ONLY known income flows + bonuses, so genuine other income (family transfers, ad-hoc credits) never showed and the card's income total undershot the Spending panel. `reconcile.js` now appends an **"Other income"** flow-group (`type:"flows"`, `noEdit:true` lines, one per source, `received:true`) built from this month's inflows that (a) don't match a known income-flow `emma_match_key` and (b) aren't refunds (`paidMerchants` = any merchant we ever pay in the full feed). `forecast.js flowLine` suppresses the edit pencil for `noEdit` lines. Pure/import-free (reuses exported `txKey`). Tests: reconcile 40→48 (**165 total green**). ⚠️ Still diverges from Spending in that refunds are excluded from income but NOT netted into the card's General (Spending nets them); minor, left as-is. The separate recurring-carve alignment (⚠️ above) is still open.

**V1 close-out batch (as-built, Session 14; v1.19.0).** Three features that conclude V1:
- **Project line items ↔ real transactions (`project_item_txns`).** A line item can link MANY Emma transactions (deposit now, balance later). New table `house_project.project_item_txns` `(item_id FK, emma_txn_id, merchant, txn_date, amount)` — migration `20260712120000_project_item_txns.sql`, applied live; unique on `(household_id, emma_txn_id)` so re-linking a txn MOVES it (upsert). Added to `store.js` TABLES/state (paged) + `linkProjectTxn(row)`/`unlinkProjectTxn(id)`. `emma.js` now parses the feed's **`ID`** column (`id`). The link UI is a "Linked transactions" section in the line-item sheet (`projects.js renderLinks` via the sheet `extra` hook, existing items only — needs an id) — search the memoised feed (outflows, not already linked to any item), tap to link/unlink. **`amount` is snapshotted** on the link row so it survives the feed's ~12-mo window. On every link change: item `actual_cost = SUM(links)` (written via `saveRow`, so the manual actual field is DROPPED from the sheet once links exist — like a project's `estimated_cost` goes read-only with line items), then `syncTotals` rolls it to the project. **Forecast impact:** `engine.js projectCostInMonth` now = `plannedCostInMonth(p,mIdx) × max(0, est−act)/est` — actuals ALREADY left the account (baked into the Emma-anchored opening cash), so they must NOT be re-deducted; they SHRINK the remaining plan the forecast still chases. `est` unchanged, `act=0` → full plan; `act≥est` → 0. Engine reads only project fields (`currentForecast` already injects derived `actual_cost` from items) — never touches `project_item_txns`. Tests: engine 84→89.
- **One-tap best-guess category (`v1.19.0`).** `categories.js guessCategory(t, knownNames)` (pure) — keyword-match the merchant name first, then Emma's raw category, **snapped to an existing category name** (`snapToKnown`: exact, else shortest known that contains/is-contained-by the guess, e.g. "Bills"→"Recurring - Bills"). Returns null when nothing matches or the txn is internal-money (passthrough). The Spending "needs a category" prompt (`spending.js uncategorisedHtml`) shows each merchant's guess as a mint **Accept ✓ chip** (`saveCategoryRule` in one tap, no sheet); the row body still opens the full sheet to pick differently. `.sp-uncat-row` is now a flex wrapper (`.sp-uncat-open` + `.sp-uncat-accept`).
- **Mobile viewport locked app-like (`v1.19.0`).** `index.html` viewport meta gained `maximum-scale=1, user-scalable=no`; `html{-webkit-text-size-adjust:100%}`; `body{touch-action:manipulation; overscroll-behavior:none; max-width:100%}`. Kills accidental pinch/double-tap zoom + rubber-band overscroll that left the PWA off-screen. ⚠️ Not device-testable in the sandbox — verified only via the standard bundle; refine if a specific overflow remains.

**Project linking flows through the app (as-built, Session 14; v1.20.0).** Follow-on to the v1.19.0 line-item↔txn linking — a project-linked transaction is now treated as **project spend, not discretionary**, everywhere:
- **(A) Transaction-side linking.** The Spending **categorise sheet** (tap any transaction/merchant) gained an **"Add to project"** picker (`spending.js renderProjectLink` via the sheet `extra` hook) — a `<select>` of project ▸ line item (optgroups; only projects with line items), one button links every currently-unlinked outflow from that merchant to the chosen item. Calls exported **`projects.linkTransactionsToItem(itemId, txns)`** (link each → recompute item `actual_cost = SUM(links)` → `syncTotals`). `synthKey` moved to `categories.js` (shared txn identity: feed `id` else `date|amount|txnKey`). So a NEW merchant can be pushed onto a project the moment it arrives, from the transaction side.
- **(B) Forecast "This month" budgets projects out of General.** `reconcile.js` now takes **`opts.projectKeys`** (Set of linked synthKeys; `forecast.js` passes `state.project_item_txns` keys) and **excludes those txns from the General Expenses actual** — a project payment no longer eats the discretionary budget. `synthK` replicated inline (import-free core). The existing planned **Projects** group in the card (engine-sourced, scaled by remaining) stays as the projects section; actuals are represented via that + opening cash (not itemised green-"paid" in the card — deliberately simple). Test: reconcile 48→50.
- **(C) Spending month panel Projects section.** `buildMonthActuals` routes project-linked outflows (checked FIRST, before invest/excluded/category) into a **Projects** section (project ▸ per-merchant NET sums), carved out of General, with a "Projects −£X" summary line, **counted in Net as spent** (`net = totIn − totOut − totProject − totInvest`). `catGroup` gained a `project` kind (amber, `.sp-catoff.project`); `monthPanelHtml` renders it between Out and Savings. Mirrors how the Forecast engine separates Projects from General/recurring.

**Emma session self-heal + Projects-as-spend card (as-built, Session 15; v1.20.1).** Two fixes:
- **Emma 401 self-heal.** Supabase access tokens last ~1h and iOS PWAs suspend the background auto-refresh, so a foregrounded app fired `emma-sheet` with an expired token → the function's `requireMember` returned **401** → "Couldn't load Emma". The function itself is healthy (verified: anon-key call → `{"error":"unauthorized"}` 401 by design; earlier calls returned 200 with data, so Google SA + sheet access are fine). Fix is client-side in `emma.js`: `invokeEmma(body)` wraps `supa.functions.invoke("emma-sheet", …)` and, on `error.context.status === 401`, calls `supa.auth.refreshSession()` once and retries; if it still 401s the refresh token is dead too → throws "Session expired — reload the app or sign in again." All Emma reads funnel through `fetchEmma` → one choke point. (Diagnose function health via `supabase functions list --project-ref …` + the Management API logs: `POST`… actually `GET /v1/projects/<ref>/analytics/endpoints/logs.all?sql=…&iso_timestamp_start=…` over `function_edge_logs` — unnest `metadata`→`response.status_code`; needs a 6h+ window or it returns []).
- **Forecast "This month" Projects group is now a SPEND group** (`forecast.js projectGroup()`), matching the recurring/General groups: **expected** = this month's planned project cost (engine `plannedCostInMonth`, **un-shrunk** — the month's budget slice), **actual** = project-linked transactions that HIT this month (from `project_item_txns`, `txn_date` parsed US "M/D/YYYY"→'YYYY-MM'), rendered via the shared `groupRow` so it gets the progress bar + per-project **paid/due** lines (expandable to the payments that landed, `noEdit:true`). Replaces the old "just the remaining figure, tagged planned" view. ⚠️ The engine's cash-line `projectCostInMonth` still uses the **shrunk** remaining (est−act) — the card's expected (un-shrunk plan) is a display choice, so a deposit that hit ahead of its planned month shows as expected £0 / actual £X (over). No engine/test change (forecast.js isn't unit-tested; 172 still green).

**V2 kickoff — nav/routing rework + Home (as-built, Session 16; v1.21.0).** V2 planning is captured in **`V2-PLAN.md`** (approved scope, IA, build sequence). Step 1 shipped:
- **Navigation is now data-driven from ONE `NAV` array in `app.js`** — objects `{id,label,icon,group,action?}`, grouped `Home / Plan / Spend / Set up`. That one list renders the desktop `#sidebar`, the mobile `#bottomnav`, AND the mobile More drawer `#drawer-list` (`renderGrouped` + `renderBottom`). Don't reintroduce the old `TABS`/`go()`/`data-tab` — nav items carry `data-nav`; a Settings item uses `action:"settings"` (opens the gear sheet, not a screen).
- **Mobile bottom nav = 5 slots** `BOTTOM = ["home","forecast","spending","projects"]` + a **More** button that opens a bottom-sheet **drawer** (the grouped menu, same as the sidebar). Reports/Merchants/Finances/Settings live only in the drawer + sidebar.
- **Hash router** (`applyRoute`/`currentView` on `hashchange`) replaces `go()`. Top-level views today; **`#/projects/:id` is the reserved sub-route slot** for the project detail page (V2 step 2). Back button works because each nav pushes a hash entry. `applyRoute()` runs at the end of boot to land on the hash (defaults to Home).
- **Home is the landing** (`js/home.js`, `#home-root`) — **three EQUAL pillars** (Forecast / Spending / Projects), each a `.hp-pillar` glass card that routes via `location.hash` on tap. Forecast + Projects read synchronously (`currentForecast()` / `state`); Spending lazily `fetchEmma()`s and fills in. Do NOT re-weight Projects heavier — equality was an explicit user call.
- **`js/reports.js` + `js/merchants.js` are STUBS** (placeholder screens) so the nav is complete — built out in later V2 steps. **Tasks** temporarily sits under the Plan group (still `mountTasks`, `#tasks-root`) until it folds into the reworked Projects page (step 2).
- CSS added to `index.html`: `.nav-group`, `.drawer*` (mobile-only, hidden ≥860px), `.hp-*` (Home pillars). New screen `<section>`s: home (active default), reports, merchants. Design was approved via a hi-fi mockup in the real dark/aurora skin before building.

**Project-linking move/per-payment fix (as-built, Session 16; v1.21.1).** Fixed two glitches in the V1 `project_item_txns` linking:
- **Per-payment linking on the transaction side.** `spending.js renderProjectLink` (the categorise sheet's "Add to project") was a single "link ALL of this merchant's payments" button. Now it lists **each** payment (date · amount) with a per-row **Link/Move** to the selected project▸item, plus an optional "link all unlinked". So one payment (e.g. a single £531 B&Q) can be linked without dragging every B&Q txn along.
- **Moving an already-linked payment.** Both surfaces used to HIDE any txn already linked to *any* item, so you could never move one (e.g. Tool Station Garage→Hallway) without first unlinking. Now `renderProjectLink` (spending) and `renderLinks` (projects line-item search) only hide txns linked to **THIS** item; ones linked elsewhere show a `.lx-linked` tag ("in <project · item>") and linking them **moves** them (the `linkProjectTxn` upsert on `emma_txn_id` already did the move at the DB layer — the UI just never surfaced it).
- **Both-item recompute on move.** `projects.js linkTransactionsToItem` now collects the source item of every moved txn and recomputes **both** source and target `actual_cost` (new `recomputeItem` helper) — previously only the target was recomputed, leaving the source overstated. The line-item search's add path routes through `linkTransactionsToItem` (not a bare `linkProjectTxn`) for the same reason. New CSS `.pi-one` / `.lx-linked` in `index.html`.

**V2 step 2 — Projects rework (as-built, Session 17; v1.22.0).** Projects are now **pages, not pop-ups**. Documented in `V2-PLAN.md` (step 2 marked shipped).
- **Routed detail page `#/projects/:id`** — new `js/project-detail.js` + `<section data-screen="project-detail">` (`#project-detail-root`). The **router** in `app.js` grew a sub-route branch: `currentRoute()` returns `{view,id}`; when `view==="projects" && id`, the **project-detail** screen is shown (nav stays highlighted on Projects) and `showProjectDetail(id)` renders it; back button works (each nav pushes a hash entry). Contents: back/header (name · category · P{n} · status/budget pills · start+duration) + edit pencil (opens the slim project sheet via `editProject(p,{fieldsOnly:true,onDone})`); **stat tiles** Budget/Spent/Remaining/Affordability (affordability from `engine.projectAffordability`); an **"across the build" month track** — the schedule + spend-across-build merged into ONE vertical list: each active month (planned span ∪ item `due_month`s ∪ months a linked payment landed) shows a **planned-vs-actual bar** (planned = `engine.plannedCostInMonth`, actual = linked `project_item_txns` whose `txn_date` "M/D/YYYY" parses to that month; actual bar goes coral when over) plus chips of line items due that month; **recent linked payments** (top 8, from `project_item_txns`); then the **reused** line-items/budget-lock/variance/cost-spread block via the now-exported **`projects.renderDetailExtra`**. The detail module `subscribe`s to state but only re-renders while its screen is active.
- **projects.js exports for reuse** (so the page duplicates no V1 logic): `itemsFor, sumEst, sumAct, hasActuals, linksFor, sumLinks, projectCost, priorityOf, activeMonths, AFFORD, STATUS_TINT, BUDGET_TINT, badge, editProject, renderDetailExtra`. `editProject(record, opts)` gained `opts.fieldsOnly` (suppress the extra block — the page renders it) and `opts.onDone`.
- **Projects TAB is now a dashboard** — `projectsDashboard()`: committed vs spent across **active** projects (`Planned/Quoted/In Progress`) + a **funded bar** = spendable cash (`accounts.available_for_projects`) vs remaining commitment (mint covers / amber ≥50% / coral short). Cards gained a **spend-progress bar** (actual vs cost, coral when over) shown in place of the duration bar once `actual>0`. **Tapping a card ROUTES to the detail page** (`location.hash = "#/projects/"+id`) — no longer opens a sheet. New-project "+" still opens `editProject({})`.
- **Tasks folded in** — the old Tasks tab is now a **"Projects | Tasks" sub-view toggle** at the top of the Projects screen. `tasks.js` render targets a passed container via **`renderTasksInto(root)`** (no own `subscribe`/`mountTasks` — projects.js is subscribed and re-calls it). Removed the `tasks` NAV item, `mountTasks`, and the `tasks` screen `<section>`. `.tk-*` CSS stays (still used). ⚠️ Circular import `projects.js ↔ tasks.js` is call-time-safe (both only use each other's exports inside functions).
- **Migration:** `20260713120000_project_items_due_month.sql` (`project_items.due_month text`, nullable, 'YYYY-MM'), applied live via the Management API. Additive — the engine never reads it; `store.js` loads it for free (`project_items` is already paged `select *`). The line-item sheet gained a **"Due"** month `<select>`. New CSS `.pd-*` (detail page) + `.pdash-*`/`.pc-prog*` (dashboard) in `index.html`. Tests unchanged (engine untouched): **172 green**.

**V2 step 3 — Merchants page (as-built, Session 18; v1.23.0).** `js/merchants.js` grew from a nav stub into the backward-looking "who you pay" view. No schema/engine change; documented in `V2-PLAN.md` (step 3).
- **Two views via a segmented toggle.** **Merchants** (default): every merchant **ranked by spend** over a rolling window (**3 / 6 / 12 / All** months), with **change vs the prior equal-length window** (mint ▼ down / coral ▲ up / amber "new" when no prior spend), a **sparkline** of monthly spend, a category filter, and a merchant search. Tap a merchant → an **in-page detail** (`selMerchant` module state, mirrors Spending's `selMonth`) with monthly outflow bars + every transaction for that merchant, tappable to re-file. **All transactions**: the filterable/sortable (date / amount / name) full-feed browser, both directions — the fuller successor to Spending's search box (Spending's search was **left intact** this step; removing it is a possible follow-up).
- **`rankMerchants(txns, {rules, excluded, monthsBack, endInt})` is a PURE, import-free core** (spend = outflows only; excluded categories skipped; multi-field rule override; window + prior-window totals → `{months:[ord…], rows:[{key,category,total,count,prev,delta,deltaPct,byMonth}]}`). Same import-free discipline as reconcile/recurring — it **replicates `effectiveCategory` inline** so the JavaScriptCore runner can bundle it. `tests/merchants.tests.js` (22 assertions) wired into `run.sh`; **194 total green** (89 engine / 33 recurring / 50 reconcile / 22 merchants). Month arithmetic uses a **month ordinal** `y*12 + (m-1)` throughout.
- **Reuse, no duplication:** the memoised Emma feed (`emma.js` `fetchEmma`/`cachedEmmaTxns`), and `categories.js` `effectiveCategory`/`txnKey`/`buildExcludedSet`/`categoryNames`. **Re-filing opens the SAME Spending sheet:** `spending.js` now **exports `categorise(key, currentCat, feed = txns, onDone = render)`** — the `feed` + `onDone` params let other tabs (Merchants passes `cachedEmmaTxns()` + its own `render`) open the exact categorise sheet incl. the project-link hook, and it works even if Spending was never mounted (its module-local `txns` would still be null). `renderProjectLink(box, key, feed=txns)` gained the same feed param. No import cycle (spending doesn't import merchants). New CSS `.mc-*` in `index.html`.

**V2 step 4 — Reports / Budgeting page (as-built, Session 19; v1.24.0).** `js/reports.js` grew from a nav stub into the backward-looking "where does the money go, and what could we cut" view. No schema/engine change; documented in `V2-PLAN.md` (step 4).
- **Category trends.** Spend by **effective category** over a rolling **3 / 6 / 12 / All**-month window (same period control + `PERIODS` pattern as Merchants), with **change vs the prior equal-length window** (mint ▼ / coral ▲ / amber "new"), a **sparkline**, and each category's **£/mo average**. Sortable **Biggest** (total desc) / **Movers** (biggest riser = `delta` desc). Above the list: a **monthly-total bar chart** (`totalsChart`, violet bars — the overall spend direction). Tap a category → in-page drill (`selCat` module state, mirrors Merchants' `selMerchant`) listing its **ranked merchants** (reuses `merchants.rankMerchants` filtered to that category), each tappable to **re-file via the shared Spending `categorise` sheet** (passes `cachedEmmaTxns()` + `render`, so it works even if Spending was never mounted).
- **Cost to kill.** Every **expense `recurring_flow`** annualised via **`annualCost(flow)`** (frequency/interval-aware: monthly ×12/n · weekly ×52/n · yearly ×1/n; legacy = monthly), biggest **£/yr** first, with a per-year total. The concrete "cut this, save £X/yr" list — sourced from the curated `recurring_flows` (confirmed commitments), NOT re-derived from raw txns. Edit/end in Finances.
- **`reportCategories(txns, {rules, excluded, monthsBack, endInt})` + `annualCost(flow)` are PURE, import-free cores** (same discipline as `rankMerchants`/`reconcile`/`recurring` — replicate `effectiveCategory` inline so the JavaScriptCore runner bundles them). → `{months:[ord…], rows:[{category,total,count,avg,prev,delta,deltaPct,byMonth}], monthlyTotals:{ord:£}, grandTotal}`. Month ordinal `y*12+(m-1)` throughout. ⚠️ **Strict categorisation applies** — an unmapped merchant is `Uncategorised` (Emma's raw category is NOT trusted), so real category grouping depends on `category_rules`; tests must supply rules to see named buckets. `tests/reports.tests.js` (32 assertions) wired into `run.sh`; **226 total green** (89 engine / 33 recurring / 50 reconcile / 22 merchants / 32 reports). New CSS `.rp-*` in `index.html` (drill-down rows reuse `.mc-row` via the 5-col `.rp-catrow`/`.rp-mrow` grid override).

**V2 rework batch (as-built, Session 20; v1.25.0 → v1.26.0).** A page-by-page pass on the user's tweak list. Three shipped so far:
- **Settings is a PAGE, not a pop-up (v1.25.0).** `js/settings.js` rewritten from an `openSheet` opener into a routed `#/settings` screen (`mountSettings` → `#settings-root`). The header **gear button is gone** (`gearBtn` removed from `index.html` + `app.js`); the nav `settings` item is now an ordinary routed screen (dropped `action:"settings"` + the `onNav` special-case). Content trimmed to **cash buffer · forecast horizon · the "General Expenses" monthly budget** (moved here from the Spending category manager — `settings.forecast_budgets["General Expenses"]`) · an integrations placeholder · About/version. **Dropped: the Scenario default control, the Salaries editor** (salaries are ordinary income `recurring_flows`, edited in Finances), **and Trading 212.** New `.set-*` CSS. Each field saves on change via `saveSettings`; the subscribe only re-renders while the screen is active (don't clobber a mid-edit input).
- **Categories is its own PAGE (v1.25.0).** `js/categories-page.js` (new) mounts the category manager (`categoryManagerHtml`/`wireCategoryManager` from `categories.js`) on a routed `#/categories` screen in the **Set up** nav group — lifted out of the foot of the Spending tab (Spending no longer renders it). It lazily `fetchEmma()`s (the manager needs the feed to enumerate categories + power move/merge). The General-budget row was removed from the manager (now in Settings).
- **Projects simplified — status/priority/category/duration ALL removed (v1.26.0).** LOCKED with the user. **A project has no status** — it stays in the forecast for its **remaining** cost (`est − actual`) and shrinks to £0 once linked transactions cover it (fully-paid → drops out automatically, no "Done" flag). **Spend timing comes from line items:** `store.currentForecast()` now **derives a `cost_spread` from each line item's `due_month`** (fallback: item→project `target_start_month`; a project with **no line items** has no spread → its whole `estimated_cost` **spikes in the start month**). The **engine still never reads `project_items`** — it consumes the derived `cost_spread`. `engine.plannedCostInMonth` dropped duration + even-split (cost_spread authoritative, else whole cost at start); **removed `PROJECT_ACTIVE_STATUSES`** + the status filter in the main loop, the spike flag, and `projectAffordability` (active months = spread keys, else `[start]`). `projectCostInMonth` (est−act shrink) unchanged. **Actuals come ONLY from linked transactions** — the manual `actual_cost` field is gone from the line-item sheet, and the `todo/quoted/done` item-status control is gone: **line-item status is DERIVED** — `projects.itemPaid(item)` = "Paid" once `sumLinks ≥ estimated_cost` (else "To pay"). Removed the **cost-spread editor** (`renderSpread`/`wireSpread` — timing is line-item-driven now) and the **priority (P badge + sort)**, **category**, **status**, **duration** UI from the project sheet, cards, and `project-detail.js` header. Project sort = Date/Cost only. Dashboard + `home.js` "active" = **remaining > 0** (not status). **Tasks** sub-view reworked: filters To pay/Paid/Over-budget (derived), read-only actuals, tap-through to link payments (no manual tick/actual input). `forecast.js` "This month" project group replicates the derived spread + dropped its status gate. `tests/engine.tests.js` rewritten to the new model (**91 assertions; 228 total green**). ⚠️ **Stale DB columns kept** (non-destructive, like old priority cols): `projects.status/priority/category/duration_months` + `project_items.status` still exist and hold old values but are **no longer read** — don't resurrect them. ⚠️ **Behaviour change:** existing projects without per-item `due_month`s now spike their whole cost in the start month (e.g. seed kitchen = £45k all in 2027-02 until due months are set). Unused CSS (`.dur-bar`, `.sp-row/.sp-in/.sp-list`, `.pc-score`) left in place. **Still queued this batch:** Home snapshot rework (this-month-vs-forecast + uncategorised actions), and a bigger mobile Forecast chart.

**V2 rework batch C — Reports + Analysis (as-built, Session 21; v1.27.0).** Step 4 of the tweak list. A **Keep / Review / Kill "decision"** now lives on **two** kinds of thing (the user's call — mortgage=Keep is a flow, Amazon/Next=Review are variable buckets): **`recurring_flows.decision`** (fixed commitments) and **`categories.decision`** (variable buckets), both nullable text = undecided. Migration `20260714120000_decision.sql`, applied live via the Mgmt API. Both tables already `select *`, so no `store.js` change; values are `keep`/`review`/`kill`/null.
- **Decision inputs.** Finances flow sheet ([finances.js](js/finances.js)) gained a `DECISION_SEG` segmented control (`—`/Keep/Review/Kill, `showIf` expense only; the `—` option carries value `null`). The **Categories page** manager ([categories.js](js/categories.js) `categoryManagerHtml`/`wireCategoryManager`) gained a per-row `.cat-decision` `<select>` — sets `categories.decision` (creates a managed row on the fly for an unmanaged Emma category, like the counts-as-spend toggle). `saveRow` does a **partial `.update`** so writing `decision` never clobbers `counts_as_spend` and vice-versa.
- **Reports cost-to-kill** ([reports.js](js/reports.js) `costToKill`) now **drops `decision==='keep'` flows** (fixed bills like the mortgage fall off), tags kill/review rows with a chip, and links to the Analysis screen (`.rp-link`).
- **Reports category drill now shows the merchant GRAPH.** `merchants.js` `merchantDetail` was extracted into an **exported `merchantDetailHtml(txns, key, rules, {backLabel})`** (Merchants uses it too, unchanged). Reports gained `selMerchant` state: category → merchant now opens the bars+txns detail (re-file from there) instead of jumping straight to the categorise sheet.
- **NEW `#/analysis` screen** (Spend group, [analysis.js](js/analysis.js), `mountAnalysis`, `<section data-screen="analysis">`, nav icon `scan-search`; drawer/sidebar only — bottom nav stays 5 slots). Merges **flows** (annualised via `annualCost`) + **counting categories** (£/mo from `reportCategories` over a 3/6/12/All window) into **Kill / Review / Undecided / Keep** sections. Each row has an **inline decision setter** (`.an-dec` mini-segmented; tapping the active one clears to null → `saveRow`). A category row (or an Emma-linked flow via `emma_match_key`) **drills** → `rankMerchants` list → `merchantDetailHtml`, re-file through the shared Spending `categorise` sheet. Presentation-only — no engine/pure-core change; **228 tests still green**. New `.an-*` + `.cat-decision` + `.rp-link`/`.rp-kill-chip` CSS in `index.html`.

**Decision → MERCHANT level (as-built, Session 22; v1.28.0).** ⚠️ **Supersedes the whole category/flow decision model above.** The user's call: Keep/Review/Kill is decided **per MERCHANT**, not per category and not per flow ("decide on Amazon/Next specifically, not the Shopping bucket"). New home for the value: **`category_rules.decision`** (nullable text `keep`/`review`/`kill`, migration `20260714130000_merchant_decision.sql`, applied live) — `category_rules` is already the per-merchant record, so no new table; `category_rules` is already paged so no `store.js` load change. **`store.saveMerchantDecision(match_key, category, decision)`** upserts on `(household_id, match_key)`, snapshotting the merchant's current effective category so writing a decision never blanks its categorisation (pass `decision=null` to clear).
- **[analysis.js](js/analysis.js) is now a pure merchant triage.** `rankMerchants` over the 3/6/12/All window → grouped **Kill / Review / Undecided / Keep**. Undecided is capped to the **top 40 by spend** (a shortlist, not the whole feed; a "+N smaller merchants" tail note shows the rest exist). **Keep is OFF the triage list** — collapsed to an `an-kept-toggle` summary (`showKept` module state) so hitting Keep visibly removes it. Rows drill to `merchantDetailHtml` (re-file from there). Decision read via a `match_key → decision` map off `state.category_rules`; write via `saveMerchantDecision(r.key, r.category, value)`. No longer imports `reportCategories`/`annualCost`.
- **REMOVED:** the per-category decision `<select>` from the Categories manager ([categories.js](js/categories.js) `categoryManagerHtml`/`wireCategoryManager`); the `DECISION_SEG` control from the Finances flow sheet ([finances.js](js/finances.js)); the entire **`costToKill()`** section from [reports.js](js/reports.js) (the user said decisions aren't needed in Reports). Reports keeps category trends + the category→merchant drill, unchanged.
- **Stale columns kept, now UNUSED (non-destructive, like the retired priority cols):** `categories.decision` + `recurring_flows.decision` still exist but nothing reads or writes them — **don't resurrect the category/flow decision model.** `annualCost` is still an exported+tested pure core in `reports.js` (unused in its own UI now). Presentation-only; **228 tests still green**.

**V2 rework batch — Home snapshot + mobile chart (as-built, Session 23; v1.29.0 → v1.30.0).** The last two items on the V2 rework list; **V2 rework batch is now COMPLETE**. UI-only, no schema/engine/pure-core change (228 tests unchanged/green).
- **Home snapshot (v1.29.0, [home.js](js/home.js)).** The three equal pillars gained two things above them. (1) A **"This month" strip** (`monthStrip`) — actual-vs-forecast: it runs the same `reconcileMonth({...})` the Forecast card uses and shows **net so far** (income actual − counting spend − project-linked actual this month) next to the engine's forecast net for `currentForecast().months[0]`, with a mint/amber/coral **on-track / income-to-come / over-budget** tag (over = any expense group `over`; pending = any income group `pendingExpected`). Before the Emma feed lands it shows a **forecast-only** line (`${signed(fnet)}` + projected end cash) so there's no empty/flash state; taps route to `#/forecast`. (2) A **"needs a category" action card** (`uncatCard`) — same detection as `spending.js` (unmapped counting outflows → effective category `Uncategorised`), **top 3 by spend** each with a one-tap **accept-guess chip** (`guessCategory` + `saveCategoryRule`, which `loadAll`s → the merchant drops off on re-render); a "See all" button routes to `#/spending` where the full prompt lives. Only shown when there are unmapped merchants. Also **fixed the Spending pillar pre-feed flash**: `loadEmma` now sets `emmaLoading=true` and **renders the loading state on mount** (was showing "Tap to view spending" while the feed was actually in flight, because it set the flag after the first render). New `.hp-ms-*` / `.hp-uc-*` / `.hp-load` CSS.
- **Bigger/legible mobile Forecast chart (v1.30.0, [forecast.js](js/forecast.js)).** The bespoke SVG chart uses a **fixed viewBox scaled to container width**, so the 720-wide desktop viewBox made everything tiny on a phone. `buildChart` now picks a **narrower/taller viewBox on mobile** — `isMobile()` = `matchMedia("(max-width:640px)")` → **W=380×H=300** (vs desktop 720×300), so it scales **UP** (~1.9× larger text, a taller plot). Fewer x-labels on mobile (`ceil(n/4)` vs `n/6`); a small mobile font bump on `.ax-lbl` / `.cf-co-*` in the 640px media block (**viewBox units — compounds** with the smaller viewBox). A **`matchMedia` `change` listener** in `mountForecast` (bound once via `breakpointBound`) re-renders **only when the breakpoint is crossed** (rotation/resize), so no resize thrash and the `sparkDraw` line draw-on isn't replayed. ⚠️ Not device-testable in the assistant sandbox (browser preview blocked) — verify on the installed PWA.

**Home fixes (as-built, Session 24; v1.30.1).** Three corrections after the user reviewed the live Home screen (all in [home.js](js/home.js), no schema/engine/pure-core change; 228 tests green). (1) **Spending pillar showed £0** — `spendingPillar` filtered `monthOf(t.date)`, but `t.date` is the raw Emma `"M/D/YYYY"` **string** while `monthOf` expects the yyyymmdd **int** (`t.dateInt`), so every txn `NaN`'d out and nothing counted. Fixed to `t.dateInt`; also **dropped the total-spend-vs-General-budget bar** (comparing total spend incl. bills to the discretionary budget was apples-to-oranges) — pillar now shows plain total counting spend + biggest category. (2) **"This month" card reframed to DISCRETIONARY spend** — was net-so-far, which compares partial-month actuals (no salary landed yet) to a full-month forecast net → always misleadingly negative early in the month. Now headlines **General Expenses actual vs the monthly budget** (from `reconcileMonth`'s General group; known bills carved out via `emma_match_key`, so it's true discretionary), within/over tag + £-left + biggest category. (3) **Forecast pillar wording** — "cash now" (available-account sum) can be **below** the buffer while the forecast **low point is above** it (upcoming income lands before any dip), which read as contradictory. New sub when `now < buffer` but the dip recovers: "Below your £X buffer now — income lifts you back above · low £Y in {month}". ⚠️ **The engine is correct, not buggy** — the large month-0 forecast net is Christine's **confirmed £14k July bonus** (income £28.2k − ~£15k expenses); the Sep dip to ~£6.6k is the **£17k "Car Payment" `life_event` (expense_change, −17000, 1 month) + £1k nursery settle**. Real household figures verified via the Management API stay DB-only (repo public).

**Home Forecast tile reframed (as-built, Session 24; v1.30.2).** The user found the Forecast pillar confusing (it led with "cash now" — today's account balance, which isn't the forecast — beside a future low point). Reframed to answer one question: **does cash dip below the buffer over the horizon, and when?** `forecastPillar` now leads with the **forecast low point** (`£X lowest`, meta `next N mo`) + a buffer verdict sub (`Stays above your £Y buffer · {month}`, or coral `Dips below your £Y buffer in {month}`; the big number goes coral too when below). Spark chart unchanged. New `.hp-big .neg` CSS. Kept it on Home rather than removing (the forecast is the app's whole purpose) — the user chose reframe over delete.

**LifeOS publish adapter (as-built, Session 25; v2.1.0).** This app is now one of four feeding a new cross-app hub PWA, **LifeOS** (separate repo `../LifeOS`, live at `https://silkham.github.io/lifeos/`). LifeOS is read-mostly: each source app publishes "signals" (metric/task/nudge rows) into a **`lifeos` schema** on the *same* shared DB (Project A `dgbbyijhabjozqrkokrq`), unique on `(household_id, app, key)`; LifeOS merges + renders them, CTA deep-links back. **This app's adapter = [js/lifeos.js](js/lifeos.js)** `publishToLifeOS()`, called fire-and-forget after boot in [app.js](js/app.js) (after `applyRoute()`). It upserts two `metric` signals — `month-net` + projected `cash` — computed from `currentForecast().months[0]`, via `supa.schema("lifeos").from("signals").upsert(rows, {onConflict:"household_id,app,key"})`. **No `house_project` schema change** — it writes to the separate `lifeos` schema (its RLS uses `lifeos.my_household_ids()`, same pattern as ours). Never blocks/breaks the app (try/catch, `console.warn` on fail). ⚠️ Full LifeOS architecture (the signal contract, the 2-Supabase-project split, other apps' adapters) lives in **`../LifeOS/CLAUDE.md`** — read that before touching the hub. Roadmap here: enrich with Emma actual-vs-forecast + emit task/nudge signals (e.g. stale-balance nudge).

**Project links survive Emma pending→posted churn (as-built, Session 26; v2.2.1 → v2.3.0).** ⚠️ **LANDMINE — Emma re-issues a transaction when it POSTS.** A pending Emma txn posts as a *different* row: new `id`, new `Merchant` (pending "SOUTHERN SHEDS LTD" → posted "Southern Sheds Ltd, Rochester"), new `Date` (7/11 → 7/13). **Only the amount is stable.** So `synthKey` (`t.id || date|amount|merchant`, [categories.js](js/categories.js)) — which is used **only** for project↔txn linking (`project_item_txns.emma_txn_id`) — is *not* durable across the pending→posted transition. Symptom: a link made while pending **orphans** (its id vanishes from the feed) → it still feeds the Forecast project line (that reads `project_item_txns` rows directly by date) while the posted txn sits **unlinked in General** = double-count; re-linking the posted version keys a *new* row = duplicate. **Fix (both in [projects.js](js/projects.js), no migration):** (Tier 1) `linkTransactionsToItem` drops an orphaned same-item/same-amount twin (id no longer in feed) when you link the posted version, so a re-link MOVES not dupes. (Tier 2) `healProjectLinks(feed)` runs on every REAL feed load via a new `onFeedLoaded(cb)` hook in [emma.js](js/emma.js) (callback so emma.js needn't import projects.js — **no cycle**): re-points an orphan to its posted twin (exact amount + ≤5 days + exactly one candidate) or drops it when a live twin is already linked to the same item. Re-entrancy-guarded (`_healing`); never de-dupes against an **unloaded** feed. Also (v2.2.1) `uncategorisedHtml` ([spending.js](js/spending.js)) + `uncatCard` ([home.js](js/home.js)) now skip project-linked txns so an allocated payment never reads as "needs a category". **Practical guidance: prefer linking payments AFTER they've posted** — Tier 2 auto-heals pending links, but a posted txn is a stable target.
