...
**Current-month reconciliation — phase 2 (as-built, Session 8; v1.5.0).** The Forecast tab's **current month** is now a live **expected-vs-actual** panel ("This month" card, between the alert strip and the monthly table) that fuses the forward-looking recurring flows with the backward-looking Emma feed, so overspend / a missing salary shows *mid-month*. Model: **groups → categories → transactions** (all collapsible). `js/reconcile.js` holds the **pure** `reconcileMonth({month, txns, recurring_flows, categories, category_rules, budgets, excluded})` → `{income:[groupNode], expense:[groupNode]}` (see `tests/reconcile.tests.js`, 25 assertions, wired into `run.sh`; 113 total green). Two group flavours: a **bill group** (Housing, Salary) takes expected from its linked recurring flows and shows **landed vs still-to-come per discrete line** (matched on `recurring_flows.emma_match_key` appearing in *this month's* feed — NOT date logic; unlanded expected rolls up as the group's `pendingExpected`); a **budget group** (reserved name **`General Expenses`**) takes expected from a **user-set monthly budget** and shows spent-so-far with an over flag. A group is a budget group iff a budget is set for its name. **Grouping is a `categories.forecast_group text`** (null = the category is its own line) — migration `20260708100000_forecast_group.sql` (applied live) which also added **`settings.forecast_budgets jsonb default '{}'`** (group-name → £/mo; only `General Expenses` uses it in v1). A recurring flow's group is derived from its `emma_match_key` merchant's most-recent Emma category (works before it lands), else its own name. The **phase-1d category manager** (bottom of Spending) gained a per-row **`forecast_group`** datalist input + a **General Expenses budget field** (placeholder = the Emma historical monthly average, auto-computed). Forecast lazily `fetchEmma()`s (memoised — shares the one Edge Function call); clicking a discrete line's pencil opens `finances.openRecurringSheet(flow, render)`. `reconcileMonth` re-runs on every Forecast render (cheap). Nothing here mutates the Emma feed or the cashflow engine — it's a read-only overlay on the current month.
