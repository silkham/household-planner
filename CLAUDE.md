...
- **Stale columns kept, now UNUSED (non-destructive, like the retired priority cols):** `categories.decision` + `recurring_flows.decision` still exist but nothing reads or writes them — **don't resurrect the category/flow decision model.** `annualCost` is still an exported+tested pure core in `reports.js` (unused in its own UI now). Presentation-only; **228 tests still green**. **Still queued (V2 rework batch):** Home snapshot rework + bigger mobile Forecast chart.
